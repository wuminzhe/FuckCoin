...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("FuckCoin"))
        return false;
...
{
    // Convert FuckCoin:// to FuckCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("FuckCoin://"))
    {
        uri.replace(0, 12, "FuckCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "FuckCoin.lnk";
}
...
{
    return GetAutostartDir() / "FuckCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("FuckCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  FuckCoin-qt [" + tr("command-line options") + "]                     " + "\n";
