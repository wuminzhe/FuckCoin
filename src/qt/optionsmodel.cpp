...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeFuckCoinAmount"))
        settings.setValue("nAnonymizeFuckCoinAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeFuckCoinAmount = settings.value("nAnonymizeFuckCoinAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeFuckCoinAmount"))
        SoftSetArg("-anonymizeFuckCoinamount", settings.value("nAnonymizeFuckCoinAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeFuckCoinAmount:
            nAnonymizeFuckCoinAmount = value.toInt();
            settings.setValue("nAnonymizeFuckCoinAmount", nAnonymizeFuckCoinAmount);
            emit anonymizeFuckCoinAmountChanged(nAnonymizeFuckCoinAmount);
            break;
        default:
            break;
        }
